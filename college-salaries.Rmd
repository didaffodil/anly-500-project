...
```{r}
BREAKEVENinstate=lapply(c(MN_EARN_WNE_P6), c(TUITIONFEE_IN), divide)
BREAKEVENoutstate=lapply(c(MN_EARN_WNE_P6), c(TUITIONFEE_OUT), divide)
```

## Current ROI USING MEAN EARNINGS of beigining salary

```{r}
diffinstate=lapply(c(MN_EARN_WNE_P6), c(TUITIONFEE_IN),diff)
CROIinstate=lapply(c(diffinstate), c(TUITIONFEE_IN),divide)
diffoutstate=lapply(c(MN_EARN_WNE_P6), c(TUITIONFEE_OUT),diff)
CROIoutstate=lapply(c(diffoutstate), c((TUITIONFEE_OUT),divide)
```
## Life time ROI USING MEAN EARNINGS of end career salary

```{r}
diffinstate=lapply(c(MN_EARN_WNE_P10), c(TUITIONFEE_IN),diff)
CROIinstate=lapply(c(diffinstate), c(TUITIONFEE_IN),divide)
diffoutstate=lapply(c(MN_EARN_WNE_P10), c(TUITIONFEE_OUT),diff)
CROIoutstate=lapply(c(diffoutstate), c((TUITIONFEE_OUT),divide)
